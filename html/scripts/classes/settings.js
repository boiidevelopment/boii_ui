...
class Settings {
    ...
    apply_container_position(selector, position, alignment = null) {
        ...
        if (alignment) {
            css_properties['align-items'] = alignment;
        }
    ...
    reset_to_defaults(section, notification_type = null) {
        ...
        if (section) {
            if (this.settings[section]) {
                if (section === 'notification' && notification_type) {
                    if (this.settings.notification[notification_type] && defaults.notification[notification_type]) {
                        this.settings.notification[notification_type] = defaults.notification[notification_type];
                        localStorage.removeItem(`notification_${notification_type}`);
                    } else {
                        console.warn(`Unknown notification type: ${notification_type}`);
                    }
                } else {
                    this.settings[section] = defaults[section] || {};
                    localStorage.removeItem('custom_'+section+'_style');
                }
            } else {
    ...
    create_text_input(label, id, value, hidden = false) {
        const hidden_class = hidden ? 'hidden' : '';
        return `
            <div class="content_item ${hidden_class}">
                <span class="menu_label">
        ...
        if (options && options.class) {
            ...
            $(selector).each(function() {
                ...
                if (settings.box_shadow) {
                    $(this).css('box-shadow', settings.box_shadow);
                }
            });
    ...
    attach_settings_listeners() {
        $(document).on('click', '#save_context_customization', () => {
            ...
            const box_shadow = $('#box_shadow_input').val();
        ...
        $(document).on('click', '#reset_context_customization', () => { this.reset_to_defaults('context') });
        ...
        $(document).on('click', '#reset_dialogue_customization', () => {  this.reset_to_defaults('dialogue') });
        ...
        $(document).on('click', '#reset_notification_customization', () => { this.reset_to_defaults('notification', this.notification_dummy) });

        $(document).on('click', 'input[name="draw_text_alignment"]', (e) => {
            $('input[name="draw_text_alignment"]').prop('checked', false);
            $(e.currentTarget).prop('checked', true);
            let alignment = $(e.currentTarget).data('value');
            this.draw_text_alignment = alignment;
            this.update_setting('draw_text_alignment', alignment);
        ...
        $(document).on('click', '#reset_draw_text_customization', () => { this.reset_to_defaults('draw_text') });
        ...
        $(document).on('click', '#reset_progress_customization', () => { this.reset_to_defaults('progress') });
    }
    ...
    toggle_context_movement() {
        ...
        if (is_movable) {
            ...
            this.toggle_grid_overlay(false);
        } else {
            this.show_dummy_context();
            this.toggle_grid_overlay(true, () => {
                this.toggle_context_movement()
            });
    ...
    generate_context_settings() {
        ...
        const box_shadow_val = current_style.box_shadow || '2px 2px 5px #0d0c0c, -2px -2px 5px #2b2a2a';
        const content = `
            ...
            <div class="content_group">
                ...
                ${this.create_text_input("box_shadow", "box_shadow_input", box_shadow_val, true)}
    ...
    generate_dialogue_settings() {
        ...
        const border_size_val = current_style.border_size ? parseInt(current_style.border_size) : 0;
        const border_rad_val = current_style.border_radius ? parseInt(current_style.border_radius) : 5;
        const anim_duration_val = current_style.animation ? parseFloat(current_style.animation.match(/(\d+(\.\d+)?)/)[0]) : 2;
    ...
    update_preview_draw_text() {
        ...
        const draw_text_html = `
            <div class="draw_text" style="background-color: ${style.background}; border: ${border}; color: ${style.text_colour}; animation: ${style.animation}; box-shadow: ${style.box_shadow}">
    ...
    toggle_draw_text_movement() {
        ...
        if (is_movable) {
            ...
            this.show_dummy_draw_text();
            this.toggle_grid_overlay(true, () => {
                this.toggle_draw_text_movement()
            });
    ...
    generate_draw_text_settings() {
        ...
        const content = `
            ...
            <div class="content_group">
                ...
                ${this.create_toggle("flex-start", "alignment_left", this.settings.draw_text_alignment, true, "draw_text_alignment")}
                ${this.create_toggle("center", "alignment_center", this.settings.draw_text_alignment, true, "draw_text_alignment")}
                ${this.create_toggle("flex-end", "alignment_right", this.settings.draw_text_alignment, true, "draw_text_alignment")}
            </div>
            <span class="group_label" data-translate="edit">Edit</span>
            <div class="content_group">
                ${this.create_drop_down("customise")}
    ...
    toggle_grid_overlay(show, callback = null) {
        const grid_overlay = $('#grid_overlay');
        if (show) {
            if (grid_overlay.length === 0) {
                const overlay = $('<div id="grid_overlay">').css({
                    position: 'fixed',
                    top: 0,
                    left: 0,
                    width: '100%',
                    height: '100%',
                    background: `url("/html/assets/images/grid_overlay.png") repeat`,
                    pointerEvents: 'none',
                    opacity: 0.8,
                    zIndex: 9999
                });
                const save_btn = $('<button>').html('<i class="fa-solid fa-door-open"></i>').css({
                    position: 'fixed',
                    bottom: '20px',
                    right: '20px',
                    fontSize: '1.5rem',
                    ...
                    cursor: 'pointer',
                    color: '#b4b4b4',
                    boxShadow: '3px 5px 5px #0d0c0c, -4px -5px 6px #4dcbc2',
                    zIndex: 10000,
                ...
                $('body').append(overlay);
                overlay.append(save_btn);
                $('.tablet_container').hide();
            } else {
                $('.tablet_container').hide();
                grid_overlay.show();
            }
        } else {
            $('.tablet_container').show();
            grid_overlay.hide();
        }
    }
    ...
    toggle_notification_movement() {
        ...
        if (is_movable) {
            ...
            this.toggle_grid_overlay(true, () => {
                this.toggle_notification_movement()
            });
            $('.notification_container').addClass('movable').draggable({
                stop: (event, ui) => {
                    const position = { top: ui.position.top, left: ui.position.left };
                    this.update_setting('notification_container_position', JSON.stringify(position));
                }
            });
        }
    }
    ...
    show_dummy_notification() {
        ...
        notif_container.css({
            ...
            'align-items': this.notification_alignment,
            'cursor': 'grab'
        });
        ...
        const notification_html = `
            <div class="notification" style="background-color: ${style.background}; border: ${style.border}; color: ${style.text_colour}; animation: ${style.animation}; box-shadow: ${style.box_shadow};">
    ...
    update_preview_notification(type) {
        ...
        const border =  `${style.border_size} ${style.border_style} ${style.border_colour}`;
        const notification_html = `
            <div class="notification" style="background-color: ${style.background}; border: ${border}; color: ${style.text_colour}; animation: ${style.animation}; box-shadow: ${style.box_shadow}">
                <div class="notification_header" style="color: ${style.text_colour};"><i class="${icon}"></i> ${header_text}</div>
    ...
    generate_notifications_settings() {
        ...
        const content = `
            ...
            <div class="content_group">
                ...
                ${this.create_drop_down("adjust_alignment")}
                ${this.create_toggle("flex-start", "alignment_left", this.settings.notification_alignment === 'flex-start', true, "notification_alignment")}
                ${this.create_toggle("center", "alignment_center", this.settings.notification_alignment === 'center', true, "notification_alignment")}
                ${this.create_toggle("flex-end", "alignment_right", this.settings.notification_alignment === 'flex-end', true, "notification_alignment")}
            </div>
            ...
            <div class="content_group">
                ...
                ${this.create_toggle("info", "dummy_info", this.settings.notification_dummy === 'info', true, "notification_dummy")}
    ...
    update_preview_progress() {
        ...
        const border = `${style.border_size} ${style.border_style} ${style.border_colour}`;
        const progress_html = `
            <div class="progress_bar" style="color: ${style.text_colour}; background-color: ${style.background}; border: ${border}; border-radius: ${style.border_radius}; animation: fade ${style.animation}; box-shadow: ${style.box_shadow};">
