class ContextManager {
    ...
    close() {
        ...
        $.post(`https://${GetParentResourceName()}/close_menu`, JSON.stringify({}));
    }
    ...
    build_header(context_menu, header_data) {
        if (header_data) {
            const header = $('<div>').addClass('menu_header');
            ...
            if (header_data.message) {
                const header_text = $('<div>').addClass('menu_header_text').text(header_data.message);
    ...
    build_option(option_item, option) {
        option_item.css('color', this.custom_style.colour || this.default_style.colour);
        switch (option.type) {
            ...
            case 'checklist': this.build_checklist_checklist_option(option_item, option); break;
    ...
    build_dropdown_option(option_item, option) {
        this.build_regular_option(option_item, option);
        ...
        option_item.on('click', function(e) {
            if (!$(this).hasClass('disabled')) {
                e.stopPropagation();
                $('.submenu').not(submenu_list).hide();
                submenu_list.toggle();
                $(this).toggleClass('dropdown_wrapper');
            }
        });
    }
            ...
            if (!option_item.hasClass('disabled')) {
                button.on('click', () => {
                    ...
                    if (option.button.action_type && option.button.action) {
                        this.trigger_event(option.button.action_type, option.button.action, values);
                    }
                });
            }
    ...
    build_colour_picker_option(option_item, option) {
        ...
        if (option.button) {
            ...
            if (!option_item.hasClass('disabled')) {
                submit_button.on('click', () => {
                    const selectedColor = color_input.val();
                    if (option.button.action_type && option.button.action) {
                        this.trigger_event(option.button.action_type, option.button.action, { selectedColor });
                    }
                });
            }
        }
    ...
    build_checklist_checklist_option(option_item, option) {
        const wrapper = $('<div>').addClass('checklist_wrapper');
        if (option.header) {
            const header = $('<div>').addClass('menu_option_header').text(option.header);
...
const test_menu = {
    ...
    options: [
        ...
        {
            type: 'toggle',
            header: 'Example Toggle Switches 2',
            message: 'This is an example toggle switch item',
            toggles: [
                { 
                    label: 'Toggle 1',
